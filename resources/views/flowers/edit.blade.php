<x-layouts.app :title="__('Editar Flor')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="bg-white dark:bg-zinc-900 rounded-xl border border-neutral-200 dark:border-neutral-700 p-8 max-w-4xl mx-auto w-full">
            ...
            <h1 class="text-3xl font-bold text-gray-800 dark:text-white mb-6">Editar Flor: {{ $flower->name }}</h1>
            ...
            <form action="{{ route('flowers.update', $flower) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $flower->name) }}"
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="price" id="price" step="0.01" value="{{ old('price', $flower->price) }}"
                ...
                <div class="mb-6">
                    <label for="stock" class="block text-gray-700 dark:text-gray-200 font-semibold mb-2">Stock <span class="text-red-500">*</span></label>
                    <input type="number" name="stock" id="stock" value="{{ old('stock', $flower->stock) }}"
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Stock actual: {{ $flower->stock }}</p>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="4"
                        ...
                        placeholder="Describe la flor...">{{ old('description', $flower->description) }}</textarea>
                    @error('description')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>

                <div class="mb-6">
                    <label for="photo_flower_url" class="block text-gray-700 dark:text-gray-200 font-semibold mb-2">Imagen de la Flor</label>

                    @if($flower->photo_flower_url)
                        <div class="mb-4">
                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">Imagen actual:</p>
                            <img src="{{ asset('storage/' . $flower->photo_flower_url) }}" alt="{{ $flower->name }}" class="w-32 h-32 object-cover rounded-lg">
                        </div>
                    @endif
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Deja vacío si no deseas cambiar la imagen</p>
                        ...
                        @forelse($categories ?? [] as $category)
                            <label class="flex items-center mb-2">
                                <input type="checkbox" name="category_ids[]" value="{{ $category->id }}"
                                    {{ in_array($category->id, old('category_ids', $flower->categories->pluck('id')->toArray())) ? 'checked' : '' }}
                ...
                <div class="flex gap-4">
                    <button type="submit" class="bg-pink-600 text-white px-6 py-3 rounded-lg hover:bg-pink-700 transition font-semibold">Actualizar Flor</button>
